...
def layer_temp(t_drop:float, t_gas:float, alpha:float=1/3) -> float:
    """
...
def sat_pressure_g(t_gas, a=77.3450, b=0.0057, c=7235, d=8.2) -> float:
    """
...
def vap_pressure_g(ppm:float, press:float, mm_gas:float, mm_drop:float) -> float:
    """
...
def eval_omega(kbtepsilon12:float, a:float=0.49924, b:float=574.81152, c=1, d:float=11267.71941, e:float=1, f:float=1.45781) -> float:
    """
...
def eval_diff_coeff(t_layer:float, omega:float, sigma12:float, p:float, mm_g:float, mm_d:float, a:float=1.86e-3, b:float=3/2, c:float=1/2, d:float=9.86923e-6, e:float=2) -> float:
    """
...
def eval_vap_heat(t_d:float, a:float=-1.66282e-7, b:float=2.53533e-4, c:float=-0.14657, d:float=35.23021, e:float=-434.96806) ->float:
    """
...
def eval_viscosity(t_layer:float, a:float, b:float) -> float:
    """
...
def eval_term(mu_g_g:float, mu_l_g:float, chi_vs:float, mm_g:float, mm_d:float) -> float:
    """
...
def eval_m_viscosity(mu_g_g:float, mu_l_g:float, chi_vs:float, mm_g:float, mm_d:float) -> float:
    """
...
def eval_cp(t_m:float, a:float, b:float, c:float, d:float, e:float, mm_x:float) -> float:
    """
...
def eval_k(t_m:float, a:float, b:float, c:float) -> float:
    """
...
def eval_phi(mu_g_g:float, mu_l_g:float, mm_g:float, mm_d:float) -> float:
    """
